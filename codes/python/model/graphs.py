...
class Graphs:
    def __init__(self, env_dim, divisor, graph_types, paths, tick_list, layers=1):
    ...
    def create_plot(self):
        ...
        if "3d" in self.graph_types: 
            for i, file_data_names in enumerate(os.listdir(self.paths[3])):
                ...
                self.plot3d.set_title('Cell proliferation at t = ' + str(self.tick_list[i]))
                ...
                output_path = os.path.join(self.paths[0], f'cell_proliferation_t{self.tick_list[i]}.png')
        ...
        if "2d" in self.graph_types and self.layers is not None:
            ...
            for i, file_data_names in enumerate(os.listdir(self.paths[4])):
                ...
                for layer in self.layers:
                    ...
                    fig.suptitle('Cell proliferation at t = ' + str(self.tick_list[i])
                    + ' for layer = ' + str(layer))
                    ...
                    # Salvo i grafici
                    output_path = os.path.join(self.paths[1], 
                    f'cell_proliferation_t{self.tick_list[i]} layer{layer}.png')
                    plt.savefig(output_path)
                    plt.close()

        # GRAFICO SUM
        if "sum" in self.graph_types:
            for i, file_data_names in enumerate(os.listdir(self.paths[5])):
            ...
            #     self.sum_plot(self.tick_list)
                ...
                # Leggo il file
                data = np.loadtxt(os.path.join(self.paths[5], file_data_names), comments='#')
                data = data.T

                fig, ax = plt.subplots()

                # Disegno le somme. plt.plot(x, y)
                plt.plot(self.tick_list, data[0], "ko-", label="Total Cells", alpha=0.7)
                plt.plot(self.tick_list, data[1], "ro-", label="Cancer Cells", alpha=0.7)
                plt.plot(self.tick_list, data[2], "go-", label="Healthy Cells", alpha=0.7)
                plt.plot(self.tick_list, data[3], "yo-", label="OAR Cells", alpha=0.1)
                ...
                # Salvo i grafici
                output_path = os.path.join(self.paths[2], f'cell_sum.png')
                plt.savefig(output_path)
                plt.close()
