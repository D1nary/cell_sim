...
Grid::Grid(int xsize, int ysize, int sources_num):xsize(xsize), ysize(ysize), oar(nullptr){
    /*
    Creazione e allocazione dinamica di matrici bidimensionali.
    Maggiori dettagli disponibili negli APPUNTI GRID, sezione DOPPIO PUNTATORE.

    - `cells = new CellList*[xsize];`
      - `cells` è un puntatore a una matrice bidimensionale.
      - `new CellList*[xsize]` crea un array di `xsize` puntatori a oggetti di tipo `CellList`.
      - `cells` rappresenta una matrice di `xsize` righe, in cui ogni elemento `cells[i]` è un puntatore a un altro array.
    
    - `for (int i = 0; i < xsize; i++) { cells[i] = new CellList[ysize]; }`
      - A ogni elemento `cells[i]` viene assegnato un array di `ysize` oggetti di tipo `CellList`.
    
    - Dopo l'allocazione, `cells` rappresenta un puntatore a una matrice bidimensionale, in cui ogni riga `cells[i]`
      è un puntatore a un array di `ysize` oggetti di tipo `CellList`.
    */
    ...
    for(int i = 0; i < xsize; i++) {
        ...
        neigh_counts[i] = new int[ysize](); //"()" inizializzano tutti gli elementi a 0
    }
    // Aggiorno il numero di neighbors all'estremità della matrice. 
    for(int i = 0; i < xsize; i++){
        neigh_counts[i][0] += 3;
    ...
    for (int i = 0; i < sources_num; i++){
        /*
        sources->add accede ai membri di un oggetto attraverso un puntatore.
        Equivalente a (*ptr).member
        */
...
int Grid::rand_adj(int x,  int y){
    int counter = 0;
    int pos[8]; // Alloco memoria per un array con 8 interi
