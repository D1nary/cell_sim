...
class Grid:
    ...
    def __init__(self, env_size, real_tumor_grid, num_hcells, num_ccells, sources, graph_types, paths, layers, oar=None):
        ...
        # Creo la griglia del vero tumore
        for k in range(self.zsize):
            for i in range(self.xsize):
                for j in range(self.ysize):
        ...
        self.sources = random_sources(self.zsize, self.xsize, self.ysize, sources)
        ...
        self.neigh_counts = np.zeros((self.zsize, self.xsize, self.ysize), dtype=int)
        ...
        for i in range(self.xsize):
            for j in range(self.ysize):
                ...
                self.neigh_counts[self.zsize - 1, i, j] += 9  # Strato inferiore
        for k in range(self.zsize):
            for i in range(self.xsize):
                # Bordi anteriori e posteriori (y = 0 e y = ysize-1)
                self.neigh_counts[k, i, 0] += 9  # Bordo anteriore
                self.neigh_counts[k, i, self.ysize - 1] += 9  # Bordo posteriore
        for k in range(self.zsize):
            for j in range(self.ysize):
                # Bordi laterali (x = 0 e x = xsize-1)
                self.neigh_counts[k, 0, j] += 9  # Lato sinistro
                self.neigh_counts[k, self.xsize - 1, j] += 9  # Lato destro

        # Correzione per gli angoli (dove ci sono meno vicini)
        self.neigh_counts[0, 0, 0] -= 3
        self.neigh_counts[0, 0, self.ysize - 1] -= 3
        self.neigh_counts[0, self.xsize - 1, 0] -= 3
        self.neigh_counts[0, self.xsize - 1, self.ysize - 1] -= 3
        self.neigh_counts[self.zsize - 1, 0, 0] -= 3
        self.neigh_counts[self.zsize - 1, 0, self.ysize - 1] -= 3
        self.neigh_counts[self.zsize - 1, self.xsize - 1, 0] -= 3
        self.neigh_counts[self.zsize - 1, self.xsize - 1, self.ysize - 1] -= 3
    ...
    def cycle_cells(self, check_data):
        ...
        if "sum" in self.graph_types and check_data:
        ...
        for k in range(self.zsize):
            for i in range(self.xsize):
                for j in range(self.ysize):  # Itera su ogni voxel nella griglia 3D
                    ...
                    if "2d" in self.graph_types and k in self.layers and check_data:
